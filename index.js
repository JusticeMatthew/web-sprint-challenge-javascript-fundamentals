...
// -- Functions first look within themselves for whatever they need, if they can't find it they will check outside themselves.
...
function lowerCaseNames(arr) {
    const tinyNames = arr.map(item => item.animal_name.toLowerCase())
...
function lowPopulationAnimals(arr) {
  const endangered = arr.filter(item => item.population < 5)
  return endangered;
}
